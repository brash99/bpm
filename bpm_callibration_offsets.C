...
void
bpm_callibration_offsets(const char* finname  = "harp_info.txt"){
  ...
  for (UInt_t ir = 0; ir < hAx.size(); ir ++){  //Loop over runs starts here
    ...
    T->SetBranchAddress("IPM3H07A.XRAW",&bpmAxraw);
    T->SetBranchAddress("IPM3H07A.YRAW",&bpmAyraw);
    T->SetBranchAddress("IPM3H07B.XRAW",&bpmBxraw);
    T->SetBranchAddress("IPM3H07B.YRAW",&bpmByraw); 
    T->SetBranchAddress("IPM3H07C.XRAW",&bpmCxraw);
    T->SetBranchAddress("IPM3H07C.YRAW",&bpmCyraw); 
    ...
    T->SetBranchAddress("IPM3H07A.XSOF",&bpmAxoff);
    T->SetBranchAddress("IPM3H07A.YSOF",&bpmAyoff);
    T->SetBranchAddress("IPM3H07B.XSOF",&bpmBxoff);
    ...
    for (Int_t iev = 0 ; iev < totev ;iev ++){
      T->GetEntry(iev);
      if (ibcm1>1){
	hbpmAxpos ->Fill(bpmAxraw);
	hbpmAypos ->Fill(bpmAyraw);
	hbpmBxpos ->Fill(bpmBxraw);
